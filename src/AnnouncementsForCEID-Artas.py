...
class Applet():
	...
	def waitingForInternetConnection(self , first_time = False) :
		...
		δύο πράγματα μόνο : "Δεν υπάρχει σύνδεση" & "Έξοδος". Αυτά τα δύο πρέπει να αφαιρεθούν λοιπόν και να αναπυτχθεί
	...
	def updates_are_available(self):
		...
		Συγκεκριμένα, κάνει "ανανέωση" στη σελίδα των ανακοινώσεων και παίρνει έπειτα τους ( νέους ίσος) συνδέσμουν που βρίσκει.
	...
	def check_for_updates(self): 
		...
		Προστήθονται οι νέες ανακοινώσεις στο μενού του applet και ειδοποιείτε ο χρήστης για την *τελευταία* ΝΈΑ ανακοίνωση.
		'''
	...
	def set_NewsItems(self) :
		...
		Καταρχήν η μέθοδος αυτή καλείτε κατά την εκκίνη της εφαρμογής. Σκόπο έχει να προσθέτει στο μενού του applet 
	...
	def set_InfoLabels(self):
		...
		Αυτά που προστίθονται από αυτη την μέθοδο είναι στάνταρ.
		'''

		if Applet.internetConnection :

			# Παμε τώρα να προσθέσουμε και άλλα πράγματα για την ευκολη πρόσβαση του χρήστη στο μενού.
	...
	def open_announcement(self, widget , announcement):
		'''
		Μέθοδος η οποία ανοίγει την εκάστοτε αναοκοίνωση στον browser του χρήστη.

		Η παράμετρος "announcement" λέει ποια ανακοίνωση να ανοίξει στον borwser του χρήστη. 
		Όταν καλείτε αυτή η μέθοδος , ανάλογα σε ποια ανακοίνη έχει κάνει "click" ο χρήστης ( 0,1,2...9 ) δείνεται ο -*- ΆΡΙΘΜΌΣ -*- αυτός,
	...
	def removeNoInternetLabels(self) :
		...
		Επειδή σε αυτή την περίπτωση το μόνο που δείχνει το applet είναι "Δεν υπάρχει σύνδεση" και το "κουμπί" εξόδου - τερματιμού του applet. 
